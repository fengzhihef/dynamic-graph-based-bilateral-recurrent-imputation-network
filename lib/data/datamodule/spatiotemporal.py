...
class SpatioTemporalDataModule(pl.LightningDataModule):
    ...
    def ensure_setup_fit(self):
        if self.torch_dataset is None:
            self.setup('fit')

    @property
    def n_nodes(self):
        self.ensure_setup_fit()
    ...
    def setup(self, stage=None): 
        ...
        if self.scale:
            ...
            eval_mask = self.torch_dataset.eval_mask.numpy()[self.train_slice]
            mask_zz_train = 1-eval_mask 
            scaler = self.get_scaler()(scaling_axis).fit(train, mask=mask_zz_train, keepdims=True).to_torch()
