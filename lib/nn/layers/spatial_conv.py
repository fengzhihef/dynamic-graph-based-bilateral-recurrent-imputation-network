...
class SpatialConvOrderK(nn.Module):
    ...
    def compute_support(adj, device=None):
        if isinstance(adj, (list)): 
            for i in range(len(adj)):
                ...
                adj[i].data = adj[i].data / (adj[i].data.sum(2, keepdims=True) + epsilon)
            return adj      
        else:
            if device is not None:
                adj = adj.to(device)
            adj_fwd, adj_bwd = adj, adj.clone()
            adj_fwd.data = adj.data / (adj.data.sum(2, keepdims=True) + epsilon)
            adj_bwd.data = adj_bwd.data / (adj_bwd.data.sum(2, keepdims=True) + epsilon)
    ...
    def forward(self, x, support): 
        ...
        if (type(support) is not list):
            support = [support]
        for a in support:
            a_truncated = torch.nn.Parameter(a.data[:x.size(0)], requires_grad=True)
            x1 = torch.einsum('ncvl,nwv->ncwl', (x, a_truncated)).contiguous()
            out.append(x1) 
            for k in range(2, self.order + 1): 
                x2 = torch.einsum('ncvl,nwv->ncwl', (x1, a_truncated)).contiguous() 
                out.append(x2) 
                x1 = x2
