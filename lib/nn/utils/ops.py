...
def reverse_tensor(tensor=None, axis=-1): #这个是反转指定维度的函数，这里给的维度是-1，也就是反转最后一个维度的数据
    if tensor is None: #如果是空，那么返回空
        return None
    if tensor.dim() <= 1: #如果维度数为1，则返回本身
        return tensor
    indices = range(tensor.size()[axis])[::-1] #维度数反向遍历，例如3，2，1，0
    indices = Variable(torch.LongTensor(indices), requires_grad=False).to(tensor.device)
    return tensor.index_select(axis, indices) #似乎就是
