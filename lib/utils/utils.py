...
def miss_mask_produce(ori_mask, missing_rate):
    shape = ori_mask.shape
    # 计算应该产生的缺失值的数量
    num_missing = int(shape[0] * shape[1] * missing_rate)
    
    # 创建一个形状为 shape 的数组，所有元素都设为 0，并且类型为 'uint8'
    mask = np.zeros(shape, dtype='uint8')
    
    # 在 mask 中随机选择 num_missing 个元素的索引
    missing_indices = np.random.choice(shape[0]*shape[1], num_missing, replace=False)
    
    # 标记这些元素为缺失（设置为 1）
    np.put(mask, missing_indices, 1)
    
    return mask

def add_miss_mask_produce(ori_mask_, missing_rate):
    ori_mask = ori_mask_.copy()  # 创建原始掩码的副本，以防止改变原始数据
    ones_idx = np.argwhere(ori_mask == 1)  # 找出原始掩码中1的位置
    # num_missing = int(ones_idx.size * missing_rate)  # 计算应产生的缺失值的数量
    num_missing = int(len(ones_idx) * missing_rate)  # 计算应产生的缺失值的数量
    # num_missing = int(ori_mask.size * missing_rate)  # 计算应产生的缺失值的数量

    # 在所有1的位置中随机选择 num_missing 个元素
    missing_indices = np.random.choice(len(ones_idx), num_missing, replace=False)

    # 在这些位置上将原始掩码中的1设置为0
    ori_mask[tuple(ones_idx[missing_indices].T)] = 0

    return ori_mask


def sample_mask(shape, p=0.002, p_noise=0., max_seq=1, min_seq=1, rng=None):
    if rng is None: #选择np的随机数生成函数。
